...
It is easy to spot regions with statistically significant changes and to observe how they change in time. 
...
**Example** Measurement plot for peptide GFGDLKSPAGL shows the measured raw data for three replicates. As this peptide is 11 amino long,  

```{r}
plot_peptide_mass_measurement(alpha_dat, sequence = "GFGDLKSPAGL")
```
...
High-resolution values not only can be presented linearly on high-resolution plot (as above), but also on 3D structure, if available. This way, the uptake values are connected with spatial 
