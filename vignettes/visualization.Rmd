...
The biggest limitation of previous methods of deuterium uptake visualization is that the results are on the peptide level. However, we offer a method of aggregation of data on peptide level into high-resolution level, using the weighted method of aggregation (as described in `vignette("statistics")`). Then, the results are presented on the heatmap.
...
**Example** Structure below is mapped with deuterium uptake values after 1 minute for eEF1B$\alpha$ state. Values are aggregated using weighted approach, color signifies no exchange (white) to high exchange (red).
