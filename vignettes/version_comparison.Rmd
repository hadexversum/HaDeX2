...
For each pair of functions in the previous section, we can assess relative execution speed using the exemplary dataset as a controlled reference for comparison.

Now we established the common functionalities for both versions of the package - there are just a few. We concentrate on four major actions: reading data file, plotting (and preparing data) uptake curve for one peptide, comparison plot of two states, and differential Woods plot with difference between two states. The two additional actions are: calculation of confidence limits and reconstruction of the peptide sequence.
...
```{r message = FALSE, echo=FALSE, results='asis', out.width="100%", fig.height=8}
