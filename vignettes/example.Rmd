...
Next, to explore the data further, we can switch the plot to 'Fractional uptake', where we use the maximal exchange control as reference. Select the checkbox 'Calculation > Fractional values' and then at 'Timepoints' make sure to select 'chosen control' for 'Deut 100% Exposure'. With these settings the Woods plot shows the differential HDX data in terms of fractional deuterium uptake differences, removing the bias of peptide length. Again, we see the same two regions stand out, and if we explore more timepoints ('Timepoints > Measurement Exposure > 2'), we find additional peptides which show rigidification, such as 570-587 (motif VI, NBD2) and 630-645 (scaffold domain). 
