...
The propagation of uncertainty is necessary when we are transforming the measured values. Here, in HDX-MS, we repeat the measurements in triplicate in order to calculate the uncertainty of mass measurement. However, when transforming mass measurements into deuterium uptake, we need to propagate mass measurement, as follows:
...
The precise equations for deuterium uptake and differential deuterium uptake for multiple calculation options are available in article `Calculations`.
...
Hybrid testing is a combination of two statistical approaches to ensure that the difference between two biological states is statistically significant. The difference is significant if two tests simultanously claims the significance.


However, it can be only achieved when we have the experiment done at least in triplicate.
...
$$interval = \frac{\sum_i^n u_c(du_n)}{i}*testvalue(k) $$
where:
